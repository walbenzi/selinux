...
describe 'selinux::default' do
  ...
  it "Check the current state of SELinux" do
    if node['force_reboot_if_needed'] == "true"
      ...
      assert_includes result, "Permissive"
    else
  ...
  it 'Verifies selinux is in permissive mode in /etc/selinux/config' do
    file('/etc/selinux/config').must_match /^SELINUX=permissive$/
